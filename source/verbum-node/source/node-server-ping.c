...
int server_ping (int sock, char *content)
{
    ...
    status = 0;
    if (strcmp(node_gconfig->information.id, dst_node_id) == 0)
        status = 1;
    ...
    nconnection = node_connection_create_item(0);
    ...
    mem_salloc(message_response, 
        strlen(message_success) + strlen(node_gconfig->node_mapper_id) + 256);

    sprintf(message_response, "%s:%s%s", 
        message_success, node_gconfig->node_mapper_id, VERBUM_EOH);
    pthread_mutex_unlock(&node_mutex_gconfig);
