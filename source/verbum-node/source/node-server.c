...
void *node_server (void *tparam)
{
    ...
    max_connections  = node_gconfig->max_connections;
    ...
    pthread_mutex_lock(&node_mutex_gconfig);
    node_gconfig->information.server_port = port;
    pthread_mutex_unlock(&node_mutex_gconfig);
...
static int prepare_workers (void)
{
    ...
    for (worker=workers; worker!=NULL; worker=worker->next) {
        node_worker_param_t *param = (node_worker_param_t *) malloc(sizeof(node_worker_param_t));
...
static void *worker_handler (void *tparam)
{
    ...
    while (1) {
        ...
        if (status == 1)
            node_process_communication(sock, 1);
