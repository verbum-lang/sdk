    ...
    while (1) {
        ...
        if (!src_node_id) {
            ...
            if (node_gconfig->information.id) {
                mem_salloc(src_node_id, strlen(node_gconfig->information.id));

                if (src_node_id) 
                    mem_scopy(node_gconfig->information.id, src_node_id);

                src_nm_port = node_gconfig->node_mapper_port;
            }
    ...
    char *dst_node_id, char *dst_nm_address, int dst_nm_port)
{
    ...
    pthread_mutex_lock(&node_mutex_gconfig);
    mem_salloc_scopy(node_gconfig->node_mapper_id, node_mapper_id);
    pthread_mutex_unlock(&node_mutex_gconfig);
