...
int node_mapper_process_communication(int sock, char *path, char *nm_id)
{
     ...
     */
        ...
        status = node_mapper_add_new_node(sock, response);

    /**
     * Ping node.
     */
    else if (strstr(response, "ping-verbum-node:"))
        status = node_mapper_update_ping_node(sock, response);

    /**
     * Ping - Connections Manager.
     */
    else if (node_mapper_check_connections_request(response))
        status = node_mapper_update_connections(sock, response);

    /**
     * Get node list.
     */
    else if (strstr(response, "get-verbum-node-list:"))
        status = node_mapper_get_node_list(sock);
     ...
     */
        ...
        status = node_mapper_delete_node(sock, response);

    /**
     * Check node exists.
     */
    else if (strstr(response, "check-verbum-node-exists:"))
        status = node_mapper_check_node_exists(sock, response);

    /**
     * Create node output connection.
     */
    else if (strstr(response, "create-verbum-node-output-connection:"))
        status = node_mapper_create_node_connection(sock, response, 0);

    /**
     * Delete connection.
     */
    else if (strstr(response, "delete-verbum-connection:"))
        status = node_mapper_delete_connection(sock, response);

    else if (strstr(response, "delete-verbum-connection-server:"))
        status = node_mapper_delete_connection_server(sock, response);
